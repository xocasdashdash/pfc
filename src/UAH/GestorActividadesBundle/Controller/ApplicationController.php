...
class ApplicationController extends Controller
{
    ...
    const APPLICATION_ERROR_MIXED_TYPE_OF_CREDITS = 9;
    const APPLICATION_EMPTY = 10;
    ...
    public function createAction(Request $request)
    {
        ...
        if ($request->isXmlHttpRequest() && $request->headers->get("X-CSRFToken", null) !== null &&
            ...
            if ($enrollments) {
                ...
                $creditsType = $this->getUser()->getCreditsType();
                foreach ($enrollments as $enrollment) {
                    $enrollment = $enrollment_repository->find($enrollment);
                    if ($creditsType !== $enrollment->getCreditsType()) {
