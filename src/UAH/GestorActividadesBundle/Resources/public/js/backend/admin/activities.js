...
$(function() {
  ...
  $('#btn_show_all_activities').on('click', function() {
    $url = Routing.generate('uah_gestoractividades_admin_activities', {
      filter: 'all'
    });
    location.href = $url;
  });
  ...
  $('#btn_print_pending_report').on('click', function() {
    ...
    var frame = $('<iframe>', {
      'src': Routing.generate('uah_gestoractividades_admin_printpending')
    }).hide();
    $('body').append(frame);
  });
  $('.filter-csv').on('click', 'a', function() {
    ...
    var frame = $('<iframe>', {
      'src': Routing.generate('uah_gestoractividades_admin_exportactivities', {
        filter: $filter
      })
    }).hide();
    $('body').append(frame);
